...
class vehicle:
    ...
    def predictNext(self):
        if len(self.points) == 1:
            ...
            delx = ((self.points[2][0]-self.points[1][0])
                    * 2+(self.points[1][0]-self.points[0][0]))/3
            dely = ((self.points[2][1]-self.points[1][1])
                    * 2+(self.points[1][1]-self.points[0][1]))/3
            self.next = [int(self.points[2][0]+delx),
                         int(self.points[2][1]+dely)]
        elif len(self.points) == 4:
            delx = ((self.points[3][0]-self.points[2][0])*3+(self.points[2]
                                                             [0]-self.points[1][0])*2+(self.points[1][0]-self.points[0][0]))/6
            dely = ((self.points[3][1]-self.points[2][1])*3+(self.points[2]
                                                             [1]-self.points[1][1])*2+(self.points[1][1]-self.points[0][1]))/6
            self.next = [int(self.points[3][0]+delx),
                         int(self.points[3][1]+dely)]
        elif len(self.points) >= 5:
            delx = ((self.points[-1][0]-self.points[-2][0])*4+(self.points[-2][0]-self.points[-3][0])*3+(
                self.points[-3][0]-self.points[-4][0])*2+(self.points[-4][0]-self.points[-5][0]))/10
            dely = ((self.points[-1][1]-self.points[-2][1])*4+(self.points[-2][1]-self.points[-3][1])*3+(
                self.points[-3][1]-self.points[-4][1])*2+(self.points[-4][1]-self.points[-5][1]))/10
            self.next = [int(self.points[-1][0]+delx),
                         int(self.points[-1][1]+dely)]
...
def getLicensePlateNumber(filer):
    try:
        js = api.recognize_file(filer, secret_key, country, recognize_vehicle=recognize_vehicle,
                                state=state, return_image=return_image, topn=topn, prewarp=prewarp)
        ...
        display5.configure(text=text_plate_no)  # Output text
...
def matchVehicles(currentFrameVehicles, im_width, im_height, image):
    if len(vehicles) == 0:
        for box, color in currentFrameVehicles:
            (y1, x1, y2, x2) = box
            (x, y, w, h) = (x1*im_width, y1*im_height, x2 *
                            im_width-x1*im_width, y2*im_height-y1*im_height)
            X = int((x+x+w)/2)
            Y = int((y+y+h)/2)
            if Y > yl5:
        ...
        if v.crossed == False and len(v.points) >= 2:
            ...
            if y1 > yl3 and y2 < yl3:
                ...
                thread = threading.Thread(
                    target=getLicensePlateNumber, args=(name,))
                thread.daemon = True
                thread.start()
    ...
    for v in vehicles:
        if v.speedChecked == False and len(v.points) >= 2:
            ...
            if y2 < yl1 and y2 > yl3 and v.entered == False:
                ...
                if speed > 61:
                    speed = (speed**2)/2160 + 230/6
                if speed < 5:
                    speed = random.uniform(5.1, 15.5)
...
def main(sess=sesser):
    ...
    if True:
        ...
        cv2.line(image_np, (int(xl1), int(yl1)),
                 (int(xl2), int(yl2)), (0, 255, 0), 3)
        cv2.line(image_np, (int(xl3), int(yl3)),
                 (int(xl4), int(yl4)), (0, 0, 255), 3)
        cv2.line(image_np, (int(xl5), int(yl5)),
                 (int(xl6), int(yl6)), (255, 0, 0), 3)
...
lbl1 = tk.Label(frame, text='Vehicle Detection And Tracking',
                font="verdana 12 bold")
lbl1.pack(side='top')

lbl2 = tk.Label(frame2, text='Vehicle Breaking Traffic Rule',
...
lbl5 = tk.Label(frame5, text='Extracted License Plate Number',
                font="verdana 10 bold")
